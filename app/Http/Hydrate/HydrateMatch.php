...
class HydrateMatch
{
    ...
    private function entityToArray(EntityMatch $entityMatch) : array
    {
        ...
        if(!empty($entityMatch->getHomeName()))
            $arr['home_name'] = $entityMatch->getHomeName();

        if(!empty($entityMatch->getAwayName()))
    ...
    private function arrayToEntity(array $arrayMatch) : EntityMatch
    {
        ...
        if (array_key_exists('away_name', $arrayMatch) && !empty($arrayMatch['away_name'])) {
            $entityMatch->setAwayName($arrayMatch['away_name']);
        }

        if (array_key_exists('home_name', $arrayMatch) && !empty($arrayMatch['home_name'])) {
