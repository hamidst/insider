...
class RepositoryMatch
{
    ...
    public function setMatchResult(EntityMatch $match) : EntityMatch
    {
        ...
        $matchArray = $this->hydrateMatch->fromEntity($match)->toArray();

        unset($matchArray['home_name']);
        unset($matchArray['away_name']);

        $this->modelMatch->newQuery()->where('id',$match->getId())->update($matchArray);
